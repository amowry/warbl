...
#define VERSION 17 //software version number (without decimal point)
...
void setup() {
  ...
  if (EEPROM.read(901) != VERSION) { //if a new software version has been loaded, update newer settings
    ...
    EEPROM.update(48, 0); //save default mode
    EEPROM.update(476, 1); //save factory setting for GHB custom vibrato
    EEPROM.update(715, 1); //save factory setting for GHB custom vibrato

    for (byte i = 0; i < 3; i++) {
      for (byte n = 0; n < 8; n++) { //update factory settings for switches
        EEPROM.update((456 + n + (i * 8)), switches[i][n]);
      }
      for (byte n = 0; n < 6; n++) {  //load the old switches settings
        switches[i][n] = EEPROM.read(56 + n + (i * 5));
      }
    }
    for (byte i = 0; i < 3; i++) { //then resave the switches settings for all three instruments, because the locations have changed in this version.
      for (byte n = 0; n < 8; n++) {
        EEPROM.update((56 + n + (i * 8)), switches[i][n]);
      }
    }
  }
