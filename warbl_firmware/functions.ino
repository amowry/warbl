...
void getExpression()
{
    ...
    if (breathMode != kPressureBreath && (ED[mode][SEND_PRESSURE] == 1 || switches[mode][SEND_AFTERTOUCH] != 0 || switches[mode][SEND_VELOCITY] == 1)) {
        useUpperBound = 100 + (ED[mode][INPUT_PRESSURE_MAX] * 8);
    }
    ...
    for (byte i = 0; i < 9; i++) {
        if (toneholeRead[i] > senseDistance && i == slideHole && stepsDown > 0) {
            if (bitRead(holeCovered, i) != 1) {
                ...
                //Serial.print("TR: ");
                //Serial.println((toneholeRead[i]-senseDistance) * toneholeScale[i], DEC);
                //Serial.print("THS: ");
                //Serial.println(toneholeScale[i], DEC);
                //Serial.print("PB: ");
                //Serial.println(iPitchBend[i], DEC);
            }
        } else {
            iPitchBend[i] = 0;
        }
...
void sendPitchbend()
{
    ...
    if (prevPitchBend != pitchBend) {
        ...
        if (noteon) {
            /*
                Serial.print("noteshift: ");
                Serial.print(noteshift);
                Serial.print(" noteplay: ");
                Serial.print(notePlaying - shift);
...
void sendNote()
{
    ...
    if (     //several conditions to tell if we need to turn on a new note.
        ...
        // need to calculate and send pressure prior to note as well, in case we are using it for velocity
        sendPressure(true);
...
void sendPressure(bool force)
{
    ...
    if (ED[mode][SEND_PRESSURE] == 1 || switches[mode][SEND_AFTERTOUCH] != 0 || switches[mode][SEND_VELOCITY] == 1) {
        ...
        if (switches[mode][SEND_VELOCITY] == 1) { //set velocity to mapped pressure if desired
            if (!noteon && switches[mode][SEND_AFTERTOUCH] == 0) { //we give more weight to velocity if it's the first note being played after silence (e.g. tonguing a note).
                velocity = m << 1;
            } else {
                velocity = m;
            }
        ...
        if (ED[mode][SEND_PRESSURE] == 1 && (m != prevCCPressure || force)) {
