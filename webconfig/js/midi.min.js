...
function sendToAll(byte2,byte3){var cc=buildMessage(byte2,byte3);var iter=midiAccess.outputs.values();for(var o=iter.next();!o.done;o=iter.next()){o.value.send(cc,window.performance.now())}}
function sendToWARBL(byte2,byte3){if(communicationMode){if(platform="app"){var cc=buildMessage(byte2,byte3);var iter=midiAccess.outputs.values();for(var o=iter.next();!o.done;o=iter.next()){o.value.send(cc,window.performance.now())}}else{if(!WARBLout){console.error("sendToWARBL: No MIDI port selected!")
return}
...
function sendFingeringSelect(row,selection){selection=parseFloat(selection);updateCells();updateCustom();blink(1);var key;if(selection==2){key=8}else if(selection==3){key=3}else if(selection==5||selection==13){key=125}else if(selection==6){key=122}else if(selection==8){key=125}else if(selection==11){key=113}else if(selection==12||selection==14||selection==15){key=123}else if(selection==17||selection==18){key=2}else if(selection==20){key=123}else{key=0}
...
function advanced(){document.getElementById("box2").style.display="block";document.getElementById("box1").style.display="none"}
...
function sendRow(rowNum){blink(1);updateCells();sendToWARBL(102,90+rowNum);var y=(100)+parseFloat(document.getElementById("row"+rowNum).value);sendToWARBL(102,y);sendMIDIrow(rowNum);sendChannel(rowNum);sendByte2(rowNum);sendByte3(rowNum);if(rowNum<3){sendMomentary(rowNum)}}
function sendMIDIrow(MIDIrowNum){blink(1);updateCells();sendToWARBL(102,90+MIDIrowNum);var y=(112)+parseFloat(document.getElementById("MIDIrow"+MIDIrowNum).value);sendToWARBL(102,y);sendChannel(MIDIrowNum);sendByte2(MIDIrowNum);sendByte3(MIDIrowNum);if(MIDIrowNum<3){sendMomentary(MIDIrowNum)}}
...
function sendVented(selection){selection=+selection;blink(1);sendToWARBL(104,40);sendToWARBL(105,selection)}
