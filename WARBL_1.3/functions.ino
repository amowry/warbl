...
void get_shift() {
  ...
  if (breathMode == kPressureBell && modeSelector[mode] != kModeUilleann){ //if we're using the bell sensor to control register
    if(bitRead(holeCovered,0) == invert[mode]) {
      shift = shift + 12; //add a register jump to the transposition if necessary.
      ...
      if(bitRead(holeCovered,8) == invert[mode]) {
...
void handleButtons() {
  ...
  if (released[0] && !pressed[2] && !pressed[1]) { //do action for button 0 release ("click") NOTE: button array is zero-indexed, so "button 1" in all documentation is button 0 here (same for others).
  ...
  //presses of individual buttons (as opposed to releases) are special cases used only if we're using buttons to send MIDI on/off messages and "momentary" is selected. We'll handle these in a separate function.
    if (justPressed[0]) { //do action for button 0 press.
    justPressed[0] = 0;
    handleMomentary(0);
  }
