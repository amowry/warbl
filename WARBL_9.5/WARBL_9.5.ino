...
#define NAF_EXPLICIT_SIZE 128 //the number of entries in the North American Flute fingering chart
...
#define VERSION 95 //software version number
...
//the three lines below can be uncommented to make a version of the software that will resave factory settings the first time it is loaded.
...
// Serial.println(timingTest); //for testing only
 // initialTime = micros(); //for testing only
  ...
  if (newState == 3 || (breathMode == kPressureBell && modeSelector[mode] != kModeUilleann && bitRead(holeCovered,0) == 0) || (breathMode == kPressureThumb && (modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic || modeSelector[mode] == kModeNAF) && bitRead(holeCovered,8) == 1)) {
...
//Serial.println(buttonPrefs[0][0][1]);Serial.println(TEST);
