...
int get_note(unsigned int fingerPattern) {
 ...
 switch (modeSelector[mode]){ //determine the note based on the fingering pattern
  ...
  case kModeRecorder: //this especially messy, should be cleaned up
...
void get_shift() {
 ...
 if (newState == 3 && !((modeSelector[mode] == kModeSax  && newNote < 62) || (modeSelector[mode] == kModeSaxBasic && newNote < 74) || (modeSelector[mode] == kModeRecorder && newNote < 76)) && !(newNote == 62 && (modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard)))  { //if overblowing (except sax in the lower register and except low D with uilleann fingering, which can't overblow)
...
void handleCustomPitchBend() {
 ...
 if (pitchBendMode == kPitchBendSlideVibrato){ //calculate slide if necessary.
   getSlide();
  }
 ...
 if (modeSelector[mode] != kModeGHB && modeSelector[mode] != kModeNorthumbrian) {//used for whistle and uilleann
  if(vibratoEnable == 1){ //if it's a vibrato fingering pattern
    if(slideHole != 2){
      iPitchBend[2] = vibratoDepth; //just assign max vibrato depth to a hole that isn't being used for sliding (it doesn't matter which hole, it's just so it will be added in later).
      iPitchBend[3] = 0;
    }
    else{
      iPitchBend[3] = vibratoDepth;
      iPitchBend[2] = 0;
    }
  } 
  ...
  if(vibratoEnable == 0b000010){ //used for whistle and uilleann, indicates that it's a pattern where lowering finger 2 or 3 partway would trigger progressive vibrato.
   ...
   if (modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic) {
    for (byte i=2; i< 4; i++) {
      ...
      else if (i != slideHole){iPitchBend[i] = 0;}
   }   
    if (iPitchBend[2] + iPitchBend[3] > vibratoDepth) {
      iPitchBend[2] = vibratoDepth; //cap at max vibrato depth if they combine to add up to more than that (just set one to max and the other to zero)
      iPitchBend[3] = 0;
    }  
  }
  ...
  if (modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard){
        ...
        if ((holeCovered & 0b100000000) == 0){
           if (bitRead(holeCovered, 3) == 1){
              ...
              if (toneholeRead[3] > senseDistance){                
                iPitchBend[3] = vibratoDepth - (((toneholeRead[3] - senseDistance) * vibratoScale[3]) >> 2);
              }
              else{
                iPitchBend[3] = vibratoDepth;
              }
    ...
    for (byte i=2; i< 4; i++) { //use holes 2 and 3 for vibrato
      ...
      if(bitRead(holeCovered, i)!=1) { //if the hole is not fully covered
       if (fingersChanged){ //if the fingering pattern has changed
        ...
        if(testNote == newNote){ //if the hole is uncovered and covering the hole wouldn't change the current note (or the left thumb hole is uncovered, because that case isn't included in the fingering chart)    
          ...
          iPitchBend[i] = 0 - vibratoDepth; //if the hole is totally uncovered, max the pitchbend
         }       
        }
      }
      ...
      if ((((iPitchBend[2] + iPitchBend[3]) * -1) > vibratoDepth) && ((slideHole != 2 && slideHole != 3) || (holeCovered & 0b100000000) == 0)){ //cap at vibrato depth if more than one hole is contributing and they add to up to more than the vibrato depth.
        iPitchBend[2] = 0 - vibratoDepth; //assign max vibrato depth to a hole that isn't being used for sliding
        iPitchBend[3] = 0;
      }    
  }
  sendPitchbend();
...
void handlePitchBend() {
 ...
 for (byte i=0; i< 9; i++) { 
  ...
  if (bitRead(holeLatched, i) == 1 && toneholeRead[i] < senseDistance){(bitWrite(holeLatched, i, 0));} //we "unlatch" (enable for vibrato) a hole if it was covered when the note was triggered but now the finger has been completely removed.
...
void sendNote(){
   ...
   if (     //several conditions to tell if we need to turn on a new note.
      ...
      !(breathMode != kPressureBell && bellSensor && holeCovered == 0b111111111)) { // don't play a note if the bell sensor and all other holes are covered, and we're not in "bell register" mode. Again, simulating a closed pipe.
         ...
         if(noteon){sendUSBMIDI(NOTE_OFF, 1, notePlaying, 64); //turn off the previous note after turning on the new one. We do it after to signal to synths that the notes are legato (not all synths will respond to this).
          } 
      ...
      if ((rx.byte1 & 0x0f) == 6) { //if we're on channel 7, we may be receiving messages from the configuration tool.
       ...
       for (byte i = 0; i < 3; i++) { //update noteshift
        ...
        if (rx.byte2 == 104) { //update receive mode, used for advanced pressure range sliders, switches, and expression and drones panel settings (this indicates the variable for which the next received byte on CC 105 will be).
          ...
          else if(pressureReceiveMode < 47) {
...
void saveSettings(byte i) {
    ...
    for (byte n = 0; n < 7; n++) {
       EEPROM.update((56 + n + (i * 6)), switches[mode][n]);
    }  
...
void loadSettingsForAllModes() {
  for (byte i = 0; i < 3; i++) {
    ...
    for (byte n = 0; n < 7; n++) {
       switches[i][n] = EEPROM.read(56 + n + (i * 6));
    }        
...
void sendSettings() {
  ...
  for (byte i = 0; i < 8; i++) {
   ...
   for (byte i = 0; i < 7; i++) { //send settings for switches in the slide/vibrato and register control panels
...
//send pressure data to configuration tool and as CC and/or velocity if those options are selected  
void sendPressure()
{
	...
	if(ED[mode][SEND_PRESSURE] == 1 || ED[mode][SEND_AFTERTOUCH] == 1 || switches[0][SEND_VELOCITY] == 1) {
  	n = constrain(map(n, minIn, (100 + (ED[mode][INPUT_PRESSURE_MAX] * 8)), 100, 900), 100, 900); //map the sensor value from the selected input range up to 100-900 before transforming, to maximize sensitivity at smaller ranges
    ...
    if(switches[0][SEND_VELOCITY] == 1){   //set velocity to mapped pressure if desired
      if(!noteon){ //we give more weight to velocity if it's the first note being played after silence (e.g. tonguing a note).
        velocity = m << 1;
      }
      else {
        velocity = m;
    ...
    if(switches[mode][SEND_AFTERTOUCH] == 1){
      sendUSBMIDI(CHANNEL_PRESSURE, ED[mode][PRESSURE_CHANNEL], m); //send MSB of pressure mapped to the output range
    }
	}
}
