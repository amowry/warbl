...
int get_note(unsigned int fingerPattern) {
 ...
 switch (modeSelector[mode]){ //determine the note based on the fingering pattern
  ...
  case kModeRecorder:
    ...
    else if ((fingerPattern) == 0b100000000){
        return 75; //if thumb is closed but all others are open play a C#
    }
    else if ((fingerPattern & 0b010000000) == 0){
        return 74; //if thumb is closed but L1 is open play a C
    }
...
void get_shift() {
  ...
  if (newState == 3 && !(modeSelector[mode] == kModeSax && newNote < 62) && !(newNote == 62 && (modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard)))  { //if overblowing (except sax in the lower register and except low D with uilleann fingering, which can't overblow)
