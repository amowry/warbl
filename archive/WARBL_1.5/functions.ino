...
int get_note() {
  ...
  if (prevHoleCovered == holeCovered) { //exit here if the pattern of covered holes hasn't changed.
    return ret; 
  }

 //everything below only happens if the pattern of covered holes has changed.

  prevHoleCovered = holeCovered;

  if (communicationMode) { //send information about which holes are covered to the Configuration Tool if we're connected.
  ...
  uint8_t tempCovered = 0;

 switch (modeSelector[mode]){ //now we'll determine the note based on teh fingering pattern
  
  case kModeWhistle: //these first two are the same
  
  case kModeChromatic:

    tempCovered = (0b011111100 & holeCovered) >> 2; //use bitmask and bitshift to ignore thumb sensor, R4 sensor, and bell sensor when using tinwhistle fingering. The R4 value will be checked later to see if a note needs to be flattened.
    ret = pgm_read_byte(&tinwhistle_explicit[tempCovered].midi_note);
    if (modeSelector[mode] == kModeChromatic && bitRead(holeCovered, 1) == 1){
  ...
  case kModeUilleannStandard: //the same as the prvious one, but we cancel the accidentals Bb and F#
    ...
    tempCovered = (0b011111110 & holeCovered) >> 1; //ignore thumb hole and bell sensor
    ...
    if (modeSelector[mode] == kModeUilleannStandard){
  ...
  case kModeRecorder:
     ...
     //If back thumb is open
      if ((holeCovered & 0b100000000) == 0){
        if ((holeCovered & 0b010000000) == 0){ //if hole L1 is also open
           return 76; //play D
        }
        return 75; //if hole L1 is closed play a C#
    }
    tempCovered = (0b011111110 & holeCovered) >> 1; //ignore thumb hole and bell sensor
  ...
  case kModeNorthumbrian:
    ...
    tempCovered = holeCovered >> 1; //bitshift once to ignore bell sensor reading
...
void get_shift() {
  ...
  if (newState == 3 && !(newNote == 62 && (modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard)))  { //if overblowing (except low D with uilleann fingering, which can't overblow)
  ...
  if (breathMode == kPressureBell && modeSelector[mode] != kModeUilleann && modeSelector[mode] != kModeUilleannStandard){ //if we're using the bell sensor to control register
    ...
    else if (breathMode == kPressureThumb && (modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic || modeSelector[mode] == kModeNAF)){ //if we're using the left thumb to control the regiser
   ...
   if(modeSelector[mode] == kModeGaita){ //Gaita requires another transposition to bring it to the correct key
    shift = shift - 1;  
    } 
...
void getExpression() {
  ...
  //calculate the center pressure value fo rthe current note, regardless of register
    ...
    else {expression = (sensorValue - halfway) * ED[mode][EXPRESSION_DEPTH];   
      }
...
void findStepsDown() {
  ...
  switch (modeSelector[mode]){
   case kModeGaita: 
     ...
     break;
   case kModeRecorder:
     stepsDown = pgm_read_byte(&steps[tempNewNote-61]);
     break;
   case kModeGHB:
   ...
   case kModeKaval:
      ...
      break;  
   default:
      stepsDown = pgm_read_byte(&steps[tempNewNote-59]);
  }
  
   slideHole = findleftmostunsetbit(holeCovered); //we also determine which is the highest uncovered hole,to use for sliding
...
void handleCustomPitchBend() {
  ...
  if ((modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic) && (vibratoEnable == 0b000010)) {
    ...
    if ((toneholeRead[3] > senseDistance) && (bitRead(holeCovered, 3) != 1)) { //if hole 3 is contributing, bend down
        pitchBend = ((toneholeRead[3] - senseDistance) * vibratoScale[3]) >> 2;

      if ((toneholeRead[2] > senseDistance) && (bitRead(holeCovered, 2) != 1)) { //if hole 2 is contributing, add that in.
        pitchBend += ((toneholeRead[2] - senseDistance) * vibratoScale[2]) >> 2; 
      }
    }   

    else if ((toneholeRead[2] > senseDistance) && (bitRead(holeCovered, 2) != 1)) { //if only hole 2 is contributing.
        pitchBend = ((toneholeRead[2] - senseDistance) * vibratoScale[2]) >> 2; 
    }
 ...
 for (byte i=0; i< 9; i++) { 
  ...
  if (pitchBendMode == kPitchBendSlideVibrato){
    if(toneholeRead[i] > senseDistance){
      if (i == slideHole && (bitRead(holeCovered, i)!=1)){
  ...
  if (bitRead(holeLatched, i) == 1 && toneholeRead[i] < senseDistance){(bitWrite(holeLatched, i, 0));} //we "unlatch" (enable for vibrato) a hole if it was covered when the note was triggered but now the finger has been compltely removed.
...
void sendNote(){
   ...
   if (     //several conditions to tell if we need to turn on a new note.
      ...
      !(switches[0][SEND_VELOCITY] && !noteon && ((millis() - velocityDelayTimer) < 20)) && // and not waiting for the pressure to rise to calculate note on velocity if we're transitioning from not having any note playing.
      ...
      !((modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard) && newNote == 62 && (bitRead(holeCovered,0) == 1))) { // and if we're in uilleann mode, don't play a note if the bell sensor is covered and all holes are covered. Again, simulating a closed pipe.
          
  
         sendUSBMIDI(NOTE_ON, 1, newNote + shift, velocity);
         if(noteon){sendUSBMIDI(NOTE_OFF, 1, notePlaying, 64); //turn off the previous note after turning on the new one. We do it after to signal to synths that the notes are legato (not all synths will respind to this).
          }    
  ...
  if(noteon){  //several conditions to turn a note off  
    if (                                                                               
      ((newState == 1 && !switches[mode][BAGLESS]) || (switches[mode][BAGLESS] && !play)) ||   //if the state drops to 1 (off) or we're in bagless mode and the sound has been turned off
       ...
       ((modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard) && newNote == 62 && (bitRead(holeCovered,0) == 1))) { //or closed uilleann pipe
           sendUSBMIDI(NOTE_OFF, 1, notePlaying, 64); //turn the note off if the breath pressure drops or if we're in uilleann mode, the bell sensor is covered, and all the finger holes are covered.
           noteon = 0; //keep track
      ...
      if ((rx.byte1 & 0x0f) == 6) { //if we're on channel 7, we may be receiving messages from the configuration tool.
       ...
       for (byte i = 0; i < 3; i++) { //update noteshift
        if (rx.byte2 == 111 + i) {
          ...
          else{noteShiftSelector[i] = - 127 + rx.byte3;}
          loadPrefs();
        }
       }
        ...
        if (rx.byte2 == 104) { //update receive mode, used for advanced pressure range sliders, switches, and expression and drones panel settings (this indicates the variable for which the next received byte on CC 105 will be).
          ...
          else if(pressureReceiveMode <46) {
...
void saveSettings(byte i) {
    ...
    for (byte n = 0; n < 6; n++) {
       EEPROM.update((56 + n + (i * 5)), switches[mode][n]);
    }  
...
void loadSettingsForAllModes() {
  for (byte i = 0; i < 3; i++) {
    ...
    for (byte n = 0; n < 6; n++) {
       switches[i][n] = EEPROM.read(56 + n + (i * 5));
    }        
...
void sendSettings() {
  ...
  for (byte i = 0; i < 8; i++) {
   ...
   for (byte i = 0; i < 6; i++) { //send settings for switches in the slide/vibrato and register control panels
...
void loadPrefs() {
      ...
      for (byte i=0; i< 9; i++) {
        ...
        if (switches[mode][CUSTOM] && ((modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic || modeSelector[mode] == kModeUilleann || modeSelector[mode] == kModeUilleannStandard) && pitchBendMode == kPitchBendVibrato)) {
          customEnabled = 1;
        }
        ...
        velocity = 64; //reset this in case it's been changed
...
void sendPressure()
{
	...
	if(ED[mode][SEND_PRESSURE] == 1 || switches[0][SEND_VELOCITY] == 1) {
 ...
 if(switches[0][SEND_VELOCITY] == 1){ 
  //if (pressureChangeRate < 1) {pressureChangeRate = 1;}
    //int m = n * pressureChangeRate; //scale the pressure by the rate of change if we're using it to send velocity.   
  velocity = constrain((map(n, 100, maxIn, minOut, maxOut)), minOut, maxOut) >> 7;
 }
