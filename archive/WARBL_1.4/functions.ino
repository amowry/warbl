...
void get_shift() {
  ...
  if (breathMode == kPressureBell && modeSelector[mode] != kModeUilleann){ //if we're using the bell sensor to control register
      ...
      if ((rx.byte1 & 0x0f) == 6) { //if we're on channel 7, we may be receiving messages from the configuration tool.
       ...
       for (byte i = 0; i < 3; i++) { //update noteshift
        ...
        if (rx.byte2 == 104) { //update receive mode, used for advanced pressure range sliders, switches, and expression and drones panel settings (this indicates the variable for which the next received byte on CC 105 will be).
          ...
          else if(pressureReceiveMode <45) {
...
void sendSettings() {
  ...
  for (byte i = 0; i < 8; i++) {
   ...
   for (byte i = 0; i < 5; i++) { //send settings for switches in the slide/vibrato and register control panels
...
void loadPrefs() {
      ...
      for (byte i=0; i< 9; i++) {
        ...
        if (switches[mode][CUSTOM] && ((modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic || modeSelector[mode] == kModeUilleann) && pitchBendMode == kPitchBendVibrato)) {
          customEnabled = 1;
        }
