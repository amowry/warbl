    ...
    for (byte i = 0; i < 9; i++) {
        ...
        if (bitRead(vibratoHoles, i) == 1 && bitRead(holeLatched, i) == 0 && (pitchBendMode == kPitchBendVibrato || i != slideHole)) { //if this is a vibrato hole and we're in a mode that uses vibrato, and the hole is unlatched
            ...
            if (pitchBendOn[i] == 1 && (bitRead(holeCovered, i) == 1)) {
                iPitchBend[i] = vibratoDepth;   //set vibrato to max downward bend if a hole was being used to bend down and now is covered
            }
        }
    }
...
void sendNote()
{
    ...
    if (     //several conditions to tell if we need to turn on a new note.
        ...
        if (noteon) {
            sendUSBMIDI(NOTE_OFF, 1, notePlaying, 64); //turn off the previous note after turning on the new one. We do it after to signal to synths that the notes are legato (not all synths will respond to this).
        }
...
void loadPrefs()
{
    ...
    for (byte i = 0; i < 9; i++) {
        ...
        vibratoScale[i] = ((8 * vibratoDepth) / (toneholeCovered[i] - 50 - senseDistance) / 2);
    } //This one is for vibrato
