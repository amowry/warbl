...
#define VERSION 10 //software version number
...
void loop() {
  ...
  if (breathMode == kPressureBell && modeSelector[mode] != kModeUilleann){
    if(bitRead(holeCovered,0) == invert[mode]) {
      shift = shift + 12;}  //add an octave jump to the transposition if necessary.
    }
 

    else if (breathMode == kPressureThumb && (modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic || modeSelector[mode] == kModeNAF)){
  ...
  if ((millis() - pitchBendTimer) >= 20){ //check pitchbend every so often
...
//for (byte i = 0; i < 9; i++) { 
 //Serial.println(modeSelector[mode]);
//}
