...
#define VERSION 90 //software version number
...
const GPIO_pin_t pins[] = {DP11,DP6,DP8,DP5,DP7,DP1,DP0,DP3,DP2}; //the digital pins used for the finger hole leds, in the following order: Bell,R4,R3,R2,R1,L3,L2,L1,Lthumb. Uses a special declration format for the GPIO library.
...
byte expressionDepth = 3; //a factor for calculating pitchBend depth with pressure expression. Can have values of around 1-8
bool expressionOn = 0;
...
void setup() {
  ...
  if (hardwareRevision == 21){ //older sensors
...
//Serial.println(lowByte(toneholeBaseline[4])));
//Serial.println(tempToneholeRead[3]);
...
//}
  //for (int i =0; i < 37; i++) { 
 //Serial.println(EEPROM.read(450));
// Serial.println(EEPROM.read(451));
//Serial.println(EEPROM.read(452));
  //}
