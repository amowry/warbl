      ...
      if ((rx.byte1 & 0x0f) == 6) { //if we're on channel 7, we may be receiving messages from the configuration tool.
        ...
        if (rx.byte2 == 102) {  //many settings are controlled by a value in CC 102 (always channel 7).
          ...
          if (rx.byte3 == 123) { //save settings as the defaults for the current instrument
            ...
            EEPROM.update(313 + mode, baglessSelector[mode]);
            ...
            EEPROM.update(274 + (mode * 2), highByte(learnedPressureSelector[mode]));
  			...
  			for (byte n = 0; n < 21; n++) {
				EEPROM.update((741 + n + (mode * 21)), ED[mode][n]);
   			} 
...
void saveFactorySettings() {
  ...
  for (byte i = 0; i < 3; i++) { //then we save all the current settings for all three instruments.
    ...
    EEPROM.update(313 + i, baglessSelector[i]);
    ...
    for (byte n = 0; n < 21; n++) {
       EEPROM.update((741 + n + (i * 21)), ED[i][n]);
    } 
      

    for (byte m = 0; m < 12; m++) {
       EEPROM.update((260 + m + (i * 20)), pressureSelector[i][m]);
    }
...
void restoreFactorySettings() {
  ...
  EEPROM.update(44, 3); //indicate factory settings have been saved
  loadFingering();
  loadCalibration();
...
//pressure date to configuration tool and as CC if that option is selected  
void sendPressure()
{
