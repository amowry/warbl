...
#define VERSION 92 //software version number
...
1,    //PRESSURE_CHANNEL
7,    //PRESSURE_CC
0,    //INPUT_PRESSURE_MIN range is from 0-127, to be scaled later up to maximum input values
100,  //INPUT_PRESSURE_MAX range is from 0-127, to be scaled later
...
{0,3,0,0,1,7,0,100,0,127,0,1,51,36,1,0,0,0,0,0,0},
...
// Serial.println(timingTest); //for testing only
 // initialTime = micros(); //for testing only
  ...
  if (newState == 3 || (breathMode == kPressureBell && modeSelector[mode] != kModeUilleann && bitRead(holeCovered,0) == 0) || (breathMode == kPressureThumb && (modeSelector[mode] == kModeWhistle || modeSelector[mode] == kModeChromatic) && bitRead(holeCovered,8) == 0)) {
...
// Serial.println(toneholeRead[0]);
 ...
 if(modeSelector[mode] == kModeGaita){
  ...
  if (                                                                               //several conditions to turn a note off
    (noteon && (newState == 1 && !bagless || (bagless && !play))) ||   //if the state drops to 1 (off) or we're in bagless mode and the sound has been turned off
